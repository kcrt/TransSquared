...
private final class AudioCaptureDelegate: NSObject, AVCaptureAudioDataOutputSampleBufferDelegate {
    ...
    func captureOutput(
        ...
        if frameLength > 0 {
            ...
            if let floatData = outputBuffer.floatChannelData {
            ...
            } else if let int16Data = outputBuffer.int16ChannelData {
                ...
                rms = sqrtf(sum / Float(frameLength))
            } else if let int32Data = outputBuffer.int32ChannelData {
                let samples = UnsafeBufferPointer(start: int32Data[0], count: frameLength)
                var sum: Float = 0
                for s in samples {
